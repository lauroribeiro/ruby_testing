...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    subject(:known_game) { described_class.new(6) }
    context 'when the previous step is 16' do
      it 'returns 10' do
        previous_step = 16
        result = known_game.subtract_random_number(previous_step)
  ...
  describe '#play' do
    context 'when the random number is 19' do
      subject(:game_one) { described_class.new(19) }
    ...
    context 'when the random number is 1' do
      subject(:game_two) { described_class.new(1) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_two.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 649' do
      subject(:game_three) { described_class.new(649) }
      # remove the 'x' before running this test
      it 'will return 7' do
