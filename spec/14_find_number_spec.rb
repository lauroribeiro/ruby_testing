...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    # let(:number_guessing) { double('random_number')}
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_guessing_one) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        expect(game_guessing_one.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_guessing_two) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        expect(game_guessing_two.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_guessing_three) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        expect(game_guessing_three.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_guessing_four) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        expect(game_guessing_four.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:game_over) { described_class.new(0, 15, correct_answer, 5) }
      # Allow the double to receive 'value' and return the same number as @guess.
      let(:correct_answer) { double('random_number', value: 5)}
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_not_over) { described_class.new(4, 12, incorrect_answer, 5) }
      let(:incorrect_answer) { double('random_number', value: 3) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      let(:edge_number) { double('random_number', value:8) }
      subject(:edge_game) { described_class.new(5, 8, edge_number, 7) }
      it 'updates min to the same value as max' do
        expect { edge_game.update_range }.to change { edge_game.min }.to(edge_game.max)
      end

      it 'does not update max' do
        expect { edge_game.update_range }.not_to change { edge_game.max }
      end
    end
  end
end
