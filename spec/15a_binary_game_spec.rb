...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    subject(:game_check) { described_class.new(3, 30)}
    # Note: #verify_input will only return a number if it is between?(min, max)

    context 'when given a valid input as argument' do
      it 'returns valid input' do
        min = game_check.instance_variable_get(:@minimum)
        max = game_check.instance_variable_get(:@maximum)
        valid_input = 5
        verified_input = game_check.verify_input(min, max, valid_input)
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        max = game_check.instance_variable_get(:@maximum)
        invalid_input = 850
        verified_input = game_check.verify_input(min, max, invalid_input)
  ...
  describe '#display_turn_order' do
    ...
    subject(:game_turn) { described_class.new(1, 100, binary_search_turn) }
    ...
    it 'increases guess_count by one' do
      expect { game_turn.display_turn_order(binary_search_turn) }.to change { game_turn.instance_variable_get(:@guess_count) }.from(0).to(1)
    end
