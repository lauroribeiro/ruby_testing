...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:cerveja) { described_class.new('cerveja') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(cerveja.type).to eq('cerveja') 
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(cerveja.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:big_drink) { described_class.new('watermelon juice',18) }
      # remove the 'x' before running this test
      it 'is full' do
        expect(big_drink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:small_drink) { described_class.new('coffe',10) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(small_drink).not_to be_full
      end
    end
  end
end
